...
void tpControlDialogImpl::OnButtonClickCreateBoundaryODAPI( wxCommandEvent& event )
{
    ...
    g_openobserver_pi->CreateBoundary(pCB);
...
void tpControlDialogImpl::OnButtonClickDeleteBoundaryODAPI(wxCommandEvent& event)
{
    ...
    g_openobserver_pi->DeleteBoundary(pDB);
}
...
void tpControlDialogImpl::OnButtonClickCreateBoundaryPointODAPI( wxCommandEvent& event )
{
    ...
    bool ret = g_openobserver_pi->CreateBoundaryPoint(pCBP);
    g_openobserver_pi->ToggleToolbarIcon();
    Show(false);
}
...
void tpControlDialogImpl::OnButtonClickDeleteBoundaryPointODAPI(wxCommandEvent& event)
{
    ...
    g_openobserver_pi->DeleteBoundaryPoint(pDBP);
}
...
void tpControlDialogImpl::OnButtonClickCreateTextPointODAPI( wxCommandEvent& event )
{
    ...
    g_openobserver_pi->CreateTextPoint(pCTP);
    g_openobserver_pi->ToggleToolbarIcon();
    Show(false);
}
...
void tpControlDialogImpl::OnButtonClickDeleteTextPointODAPI(wxCommandEvent& event)
{
    ...
    g_openobserver_pi->DeleteTextPoint(pDTP);
}
...
void tpControlDialogImpl::tpControlOnClickImportJSON( wxCommandEvent& event )
{
    ...
    if(m_checkBoxSaveJSON->IsChecked()) {
        ...
        g_openobserver_pi->m_fnOutputJSON = m_filePickerOutputJSON->GetPath();
    } else {
        g_openobserver_pi->m_fnOutputJSON = wxEmptyString;
    }

    if(g_openobserver_pi->m_fnInputJSON.GetFullPath() == wxEmptyString) {
        OCPNMessageBox_PlugIn( NULL, _("No file specified for input"), _("File not found"), wxICON_EXCLAMATION | wxCANCEL );
        return;
    }
    g_openobserver_pi->m_fnInputJSON = m_filePickerInputJSON->GetPath();

    g_openobserver_pi->ImportJSONFile();
    g_openobserver_pi->ToggleToolbarIcon();
    Show(false);
}
...
{
    if(m_checkBoxSaveJSON->IsChecked()) g_openobserver_pi->m_bSaveIncommingJSONMessages = true;
    else g_openobserver_pi->m_bSaveIncommingJSONMessages = false;
}

void tpControlDialogImpl::OnCheckBoxCloseSaveFileAfterEachWrite( wxCommandEvent& event )
{
    g_openobserver_pi->UpdateCloseAfterSave(m_checkBoxCloseAferWrite->IsChecked());
    if(m_checkBoxCloseAferWrite->IsChecked()) g_openobserver_pi->m_bCloseSaveFileAfterEachWrite = true;
    else g_openobserver_pi->m_bCloseSaveFileAfterEachWrite = false;
}

void tpControlDialogImpl::OnCheckBoxAppendToFile(wxCommandEvent& event)
{
    g_openobserver_pi->UpdateAppendToFile(m_checkBoxAppendToFile->IsChecked());
    if(m_checkBoxAppendToFile->IsChecked()) g_openobserver_pi->m_bAppendToSaveFile = true;
    else g_openobserver_pi->m_bAppendToSaveFile = false;
}

void tpControlDialogImpl::OnCheckBoxDeleteFromConfig(wxCommandEvent& event)
{
    g_openobserver_pi->m_bRecreateConfig = m_checkBoxRecreateConfig->IsChecked();
}
